...
def read_test_data_to_ndarray(filname="../data/test.csv", limit=None):
    ...
    for row in csv_reader:
        ...
        if index == 1:
            continue
        data.append(numpy.float32(row)/255)
        labels.append(10)
        if limit != None and index == limit + 1:
            break
    data_x=numpy.asarray(data)        
    data_y=numpy.asarray(labels,dtype=numpy.int32)
...
def load_data(dataset):
    ...
    test_set = read_test_data_to_ndarray("../data/test.csv",limit=28000)
    valid_set = read_data_to_ndarray("../data/valid.csv",limit=21000)
    train_set = read_data_to_ndarray("../data/train.csv",limit=21000)
